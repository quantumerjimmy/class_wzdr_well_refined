# This file only outputs the sound speed of the wzdr radiation.
...
z_max_pk = 4600000       # highest redshift involved
...
w_T_at_a_interp = interp1d(1/(1+background_z),w_wzdr)
w_T_at_a_list = w_T_at_a_interp(a_tau)

w_T_at_a_prime_interp = UnivariateSpline(1/(1+background_z),w_wzdr,s=0).derivative()
...
#             /(background_rho_cdm_tau(tau[i])+(1+1/3)*background_rho_nu_tau(tau[i]))
    c2eff_d[i,:] = cs_2_wzdr_tau(tau[i])          # Produce cs_2_wzdr
